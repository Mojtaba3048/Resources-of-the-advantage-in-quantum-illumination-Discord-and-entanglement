...
ax = fig.add_subplot(111)  # Create main axes

# Transpose because of how numpy stores axes
mesh = ax.pcolormesh(x_edges, y_edges, stat.T, shading='auto', cmap='hot')

ax.set_xlabel('Entanglement of formation', fontsize=20)
ax.set_ylabel('Quantum Advantage', fontsize=20)
ax.set_title('Max $\delta_{in}$ per mesh cell')

# Add colorbar to the main axes
cbar = fig.colorbar(mesh, ax=ax, label='Min $\delta_{in}$')

# Data for inset
oo = []
for i in range(1, 21):
    oo.append(i/20)
op = [0.022, 0.03, 0.04, 0.05, 0.06, 0.075, 0.09, 0.105, 0.125, 0.145, 0.170,
      0.2, 0.230, 0.265, 0.31, 0.36, 0.42, 0.51, 0.64, 0.83]

# Create inset axes
axin = ax.inset_axes([0.15, 0.7, 0.3, 0.3])
axin.plot(oo, op)
axin.set_xlabel(r'$\eta$')
axin.set_ylabel(r'$E_c$')
