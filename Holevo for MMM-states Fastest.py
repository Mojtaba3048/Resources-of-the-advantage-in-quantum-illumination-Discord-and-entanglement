...
l = 25            # range for generating states
...
holevos = np.array(holevo, dtype=float)
c1s = np.array(c1ss, dtype=float)
c2s = np.array(c2ss, dtype=float)
c3s = np.array(c2ss, dtype=float)
c3s = np.array(c3ss, dtype=float)

fig = plt.figure()
ax = fig.add_subplot(projection='3d')

img = ax.scatter(c1s, c2s, c3s, c=holevos, cmap=plt.hot(), s=50, vmin=0, vmax=0.06)
fig.colorbar(img)
